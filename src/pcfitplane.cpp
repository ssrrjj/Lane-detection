...
*/
  plane_model[0] = coefficients->values[0];
  plane_model[1] = coefficients->values[1];
  plane_model[2] = coefficients->values[2];
  plane_model[3] = coefficients->values[3];
  //custom_pcshow(select(cloud, inliers));
  indset.clear();
  for (int i = 0; i < inliers->indices.size(); i++) {
